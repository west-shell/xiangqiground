...
function buildSvg() {
  ...
  // 外框
  lines.push(`<rect x="${PAD - MARGIN}" y="${PAD - MARGIN}" width="${(COLS - 1) * CELL + 2 * MARGIN}" height="${(ROWS - 1) * CELL + 2 * MARGIN}" fill="none" stroke="${LINE}" stroke-width="3"/>`);
  lines.push(`<rect x="${PAD}" y="${PAD}" width="${(COLS - 1) * CELL}" height="${(ROWS - 1) * CELL}" fill="none" stroke="${LINE}" stroke-width="1"/>`);

  // 横线
  for (let y = 0; y < ROWS; y++) {
    const yy = PAD + y * CELL;
    lines.push(`<line x1="${PAD}" y1="${yy}" x2="${PAD + (COLS - 1) * CELL}" y2="${yy}" stroke="${LINE}" stroke-width="1"/>`);
  }

  // 上半部竖线
  for (let x = 0; x < COLS; x++) {
    const xx = PAD + x * CELL;
    lines.push(`<line x1="${xx}" y1="${PAD}" x2="${xx}" y2="${PAD + 4 * CELL}" stroke="${LINE}" stroke-width="1"/>`);
  }

  // 下半部竖线
  for (let x = 0; x < COLS; x++) {
    const xx = PAD + x * CELL;
    lines.push(`<line x1="${xx}" y1="${PAD + 5 * CELL}" x2="${xx}" y2="${PAD + 9 * CELL}" stroke="${LINE}" stroke-width="1"/>`);
  }

  // 九宫斜线
  lines.push(`<line x1="${PAD + 3 * CELL}" y1="${PAD}" x2="${PAD + 5 * CELL}" y2="${PAD + 2 * CELL}" stroke="${LINE}" stroke-width="1"/>`);
  lines.push(`<line x1="${PAD + 5 * CELL}" y1="${PAD}" x2="${PAD + 3 * CELL}" y2="${PAD + 2 * CELL}" stroke="${LINE}" stroke-width="1"/>`);
  lines.push(`<line x1="${PAD + 3 * CELL}" y1="${PAD + 7 * CELL}" x2="${PAD + 5 * CELL}" y2="${PAD + 9 * CELL}" stroke="${LINE}" stroke-width="1"/>`);
  lines.push(`<line x1="${PAD + 5 * CELL}" y1="${PAD + 7 * CELL}" x2="${PAD + 3 * CELL}" y2="${PAD + 9 * CELL}" stroke="${LINE}" stroke-width="1"/>`);

  // 楚河汉界
  const riverY = PAD + 4.5 * CELL;
  lines.push(`<text x="${PAD + 1.5 * CELL}" y="${riverY}" font-size="${CELL * 0.6}" fill="${LINE}" text-anchor="middle" dominant-baseline="middle" font-family="serif" dy="0.1em">楚 河</text>`);
  lines.push(`<text x="${PAD + 6.5 * CELL}" y="${riverY}" font-size="${CELL * 0.6}" fill="${LINE}" text-anchor="middle" dominant-baseline="middle" font-family="serif" dy="0.1em">漢 界</text>`);
  ...
  for (const [sx, sy] of starPositions) {
    ...
    const d = CELL * 0.15;
    const d2 = CELL * 0.08;
    lines.push(`<path d="M ${cx - d2},${cy - d - d2} v ${d} h ${-d} M ${cx + d2},${cy - d - d2} v ${d} h ${d} M ${cx + d2},${cy + d + d2} v ${-d} h ${d} M ${cx - d2},${cy + d + d2} v ${-d} h ${-d}" stroke="${LINE}" stroke-width="1" fill="none"/>`);
  }
  ...
  for (const [ex, ey] of edgeStars) {
    ...
    const d2 = CELL * 0.08;
    if (ex === 0) {
      lines.push(`<path d="M ${cx + d2},${cy - d - d2} v ${d} h ${d} M ${cx + d2},${cy + d + d2} v ${-d} h ${d}" stroke="${LINE}" stroke-width="1" fill="none"/>`);
    } else {
      lines.push(`<path d="M ${cx - d2},${cy - d - d2} v ${d} h ${-d} M ${cx - d2},${cy + d + d2} v ${-d} h ${-d}" stroke="${LINE}" stroke-width="1" fill="none"/>`);
    }
  }
